...
async function destroyUser(id: number) {
    const result = await confirmDelete('users.confirm_delete');
    ...
    <Head :title="'users.title'" />
        ...
        >
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <h1 class="text-xl font-semibold">{{ 'users.title' }}</h1>
                <Button as-child class="w-full sm:w-auto">
                    <Link href="/admin/users/create">
                        <Plus class="mr-2 h-4 w-4" />
                        <span class="sm:hidden">{{ 'users.create_title' }}</span>
                        <span class="hidden sm:inline">{{ 'users.create_title' }}</span>
                    </Link>
                </Button>
            </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left text-sm">
                        ...
                        <tr>
                            ...
                            <th class="px-4 py-3">{{ 'common.email' }}</th>
<!--                            <th class="px-4 py-3">{{ 'common.phone' }}</th>-->
                            <th class="px-4 py-3">{{ 'common.role' }}</th>
                            <th class="px-4 py-3">{{ 'common.created_at' }}</th>
...
<!--                            <td class="px-4 py-3 text-muted-foreground">{{ user.country_code }} {{ user.phone }}</td>-->
            ...
            <div class="space-y-3 md:hidden">
                ...
                >
                    ...
                    <div class="space-y-2 text-sm">
                        <div v-if="user.phone" class="flex justify-between">
                            <span class="text-muted-foreground">{{ 'common.phone' }}:</span>
                            <span class="font-medium">{{ user.country_code }} {{ user.phone }}</span>
                        </div>
                        <div v-if="user.roles.length > 0" class="border-t pt-2">
                            <p class="text-xs text-muted-foreground mb-1">{{ 'common.role' }}:</p>
                            <div class="flex flex-wrap gap-1">
                                <Badge
                                    v-for="role in user.roles"
                                    :key="role.id"
                        ...
                        <div class="border-t pt-2">
                            <span class="text-xs text-muted-foreground">{{ 'common.created_at' }}: {{ user.created_at }}</span>
                        </div>
                    </div>
                ...
                <div v-if="users.data.length === 0" class="rounded-lg border border-dashed bg-muted/20 p-8 text-center">
                    <p class="text-muted-foreground">{{ 'users.no_users' }}</p>
                </div>
            </div>
